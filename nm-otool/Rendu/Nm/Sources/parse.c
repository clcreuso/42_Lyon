...
/*   Updated: 2019/08/08 12:18:18 by clcreuso    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
void		print_binary(t_bin *list, t_bin *new)
{
	...
	while (ptr != new)
	{
		if (ptr->sect.filetype == new->sect.filetype)
...
void		parse_bin_arch(t_file *file, t_bin *binary)
{
	...
	index = 0;
	ptr = binary;
	while (++index < file->stat.st_size - 20)
	{
		if (!(file->buf = overpass_file(file, binary->start, index)))
			break ;
		if (initialize_flags_struct(NULL, *(uint32_t*)file->buf))
		{
			if ((ptr->next = initialize_bin_struct(file)))
				parse_file(file, ptr->next);
			ptr = ptr->next;
			ptr->arch_name = arch_name((char*)file->buf);
		}
	}
...
void		parse_file(t_file *file, t_bin *binary)
{
	...
	else if (binary->flags.is_arch_bin)
	...
	else
	{
		...
		{
			read_header(binary);
			read_segment_command(file, binary);
			if (!file->error && (symtab = find_symtab_command(binary)))
				read_symtab_command(symtab, binary);
		}
		if (binary->flags.is_64_bin)
		{
			...
			read_segment_command(file, binary);
			if (!file->error && (symtab = find_symtab_command(binary)))
				read_symtab_command_64(symtab, binary);
		}
	}
}
